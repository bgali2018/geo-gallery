...
class PresetListComponent extends React.Component {
  constructor(props) {
    ...
    this.getImagesFromSelectedLocation = this.getImagesFromSelectedLocation.bind(
      this
    );
  }
  ...
  getImageList = () => {
    ...
    data.pageNumber = this.state.pageNumber
  ...
  getImagesFromSelectedLocation = e => {
    let selectedAddress = this.state.presetAddressList[
      ...
      <div className="row">
            ...
            {presetAddressList.map(address => {
              return (
                <li
                  className="col-sm-12 "
                  key={address.id}
                  onClick={this.getImagesFromSelectedLocation}
                  data-id={address.id}
                  style={{ padding: "1px" }}
                >
        ...
        {this.state.photoList.length === 0 && (
          <DeafultLatLang
            ...
            default={this.state.initialLayout}
          />
        )}
